...
class Unet2D:
    ...
    def attention(tensor, att_tensor, n_filters=512, kernel_size=[1, 1]):
        g1 = tf.keras.layers.Conv2D(tensor, n_filters, kernel_size=kernel_size)
        x1 = tf.keras.layers.Conv2D(att_tensor, n_filters, kernel_size=kernel_size)
        net = tf.add(g1, x1)
        net = tf.nn.relu(net)
        net = tf.keras.layers.Conv2D(net, 1, kernel_size=kernel_size)
...
#     @tf.function
    def _createModel(self, training):
        ...
        t_bot = tf.keras.layers.Conv2D(2**self.n_levels * self.n_features, 3, padding="valid",
                   kernel_initializer=self.k_init, name="conv_d{}a-b".format(self.n_levels))(t)
        t_bot = tf.keras.layers.LeakyReLU(alpha=self.relu_alpha)(t_bot)
        t_bot = tf.keras.layers.Conv2D(2**self.n_levels * self.n_features, 3, padding="valid",
                   kernel_initializer=self.k_init, name="conv_d{}b-c".format(self.n_levels))(t_bot)
        t_bot = tf.keras.layers.LeakyReLU(alpha=self.relu_alpha)(t_bot)
        pad = 8
        ...
        for l in range(self.n_levels - 1, -1, -1):
            ...
            if self.upsample:
                t_up = tf.keras.layers.UpSampling2D(size=(2, 2), name=name)(t_bot)
            else:
                t_up = tf.keras.layers.Conv2DTranspose(2**np.max((l, 1)) * self.n_features, (2, 2), strides=2,
                                    padding='valid', kernel_initializer=self.k_init, name=name)(t_bot)
                t_up = tf.keras.layers.LeakyReLU(alpha=self.relu_alpha)(t_up)
            t = attention(t, t_up, 2**np.max((l, 1)) * self.n_features)
